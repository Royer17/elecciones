			...
			<div id="20190011135391" class="card mt-2">
				...
				<div class="card-body">
								...
								@if($detail['status'] == 1)
									...
									@if($generated_status_quantity < 2)
										<tr id="tr_2468827" class="">
											...
											@if($key == 0)
											<td id="th_2468827" scope="row">{{ $detail['office_origen'] ? $detail['office_origen']['name'] : 'WEB' }}</</td>
											@else
											<td id="th_2468827" scope="row">{{ $detail['office_origen']['name'] }}</td>
											@endif
											<td id="th_2468827" scope="row">{{ $detail['office'] ? $detail['office']['name'] : "Varias oficinas" }}</td>
					...
					<!-- <div class="text-right mt-4">
						<a href="/admin/registrados" class="btn btn-outline-primary">Regresar</a>
					</div> -->

	@endforeach

	@foreach($orders_from_multiple as $key => $simple_record)
		@php
			$office_destination = $simple_record['order']['details'][0]['office']['name'];
		@endphp
					<div class="table-responsive table_solicitud">
						<table class="table table-striped table-bordered table-condensed table-hover table-sm">
							<caption style="caption-side:top;">Dirigido a: {{ $office_destination }}</caption>
							<thead class="thead-dark">
								<tr>
									<th>#</th>
									<th>Hoja de Ruta</th>
							...
							@foreach($simple_record['order']['details'] as $key => $detail)
								@if($detail['status'] == 1)
									@php
										$generated_status_quantity++;
									@endphp
									...
									@if($generated_status_quantity < 2)
										<tr id="tr_2468827" class="">
											<td id="th_2468827" scope="row">{{ $key+1 }}</td>
											<td id="th_2468827" scope="row">{{ $simple_record['order']['code'] }}</td>
											@if($key == 0)
											<td id="th_2468827" scope="row">{{ $detail['office_origen'] ? $detail['office_origen']['name'] : 'WEB' }}</td>
											@else
											<td id="th_2468827" scope="row">{{ $detail['office_origen']['name'] }}</td>
...
<script type="text/javascript">
	$(document).ready(function(){
			...
			$("#barra_detalle").html("<div class='col-auto'><a href='/admin/registrados' class='btn btn-warning'><b>Regresar</b></a></div>");
	});
</script>
@stop
