...
<div class="row">
	...
	<div class="col-md-12">
		<h3 class="font-bold">Listado de Trámites CC</h3>
		      ...
		      <div class="card-body py-1">
						<div class="row align-items-end">
							<div class="col-md-4 mb-3">
								{!! Form::open(array('url'=>'admin/de-conocimiento','method'=>'GET','autocomplete'=>'off','role'=>'search')) !!}
							...
							<div class="col-sm-auto mb-3">
								<a href="/admin/de-conocimiento" class="btn btn-success">Reset</a>
							</div>
			...
			<table class="table table-striped table-bordered table-condensed table-hover table-sm">
				...
				@foreach ($orders as $key => $cat)
					<tr>
						...
						<td>{{ $cat->number ? $cat->number : "S/N"}}</td>
						...
						<td>
							@if($cat->status != 8)
							<a href="#" data-index="{{ $cat->id }}" class="btn btn-secondary text_option py-0 px-1 answer-solicitude-cc d-none" data-target="#modal-answer-cc" data-toggle="modal"><i class="fa-solid fa-file-pen notPointerEvent"></i>
							Responder</a>
							@endif
							@if($cat->status == 5)
							<a href="" data-index="{{ $cat->id }}" data-new_status="6" data-target="#modal-delete-" data-toggle="modal" class="btn btn-info action py-0 px-1" title="RECIBIR"><i class="fa-regular fa-envelope-open notPointerEvent"></i></a>
							@endif
							...
							<a href="" data-index="{{ $cat->id }}" class="btn btn-primary solicitude__see py-0 px-1" title="Seguimiento"><i class="fa fa-search-plus"></i></a>

							@if($admin)
							<a href="" data-index="{{ $cat->id }}" class="btn btn-danger solicitude__delete py-0 px-1 d-none" title="Eliminar"><i class="fa fa-trash"></i></a>
							@endif
						</td>
					</tr>
...
@include('almacen.solicitude.modal_answer_cc')


@push ('scripts')
<script>
$('#liSolicitudes').addClass("treeview active");
$('#liCC').addClass("active");
...
$('input[name="dates"]').on('apply.daterangepicker', function(ev, picker) {
	...
	location.replace(`/admin/de-conocimiento?inicio=${startDate}&fin=${endDate}`);
});
...
document.querySelector('.answer-solicitude-cc')
	.addEventListener('click', (e) => {
		document.querySelector('#modal-answer-cc input[name="order_id"]').value = e.target.dataset.index;
	});


document.querySelector(`#solicitude__answer`)
	.addEventListener('click', () => {
		Swal.fire({
		  title: '¿Está seguro?',
		  text: "Va a responder a la solicitud/CC",
		  icon: 'warning',
		  showCancelButton: true,
		  confirmButtonColor: '#3085d6',
		  cancelButtonColor: '#d33',
		  ...
		  if (result.value) {
		  	...
		  	const route = `/admin/solicitude-answer-cc`;

		  	const formData = new FormData(document.querySelector('#modal-answer-cc form'));
