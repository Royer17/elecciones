...
<div class="row">
	...
	<div class="col-md-12">
		<h3 class="font-bold">Listado de Trámites registrados</h3>
		      ...
		      <div class="card-body py-1">
						<div class="row align-items-end">
							<div class="col-md-4 mb-3">
								{!! Form::open(array('url'=>'admin/registrados','method'=>'GET','autocomplete'=>'off','role'=>'search')) !!}
							...
							<div class="col-sm-auto mb-3">
								<a href="/admin/registrados" class="btn btn-success">Reset</a>
							</div>
			...
			<table class="table table-striped table-bordered table-condensed table-hover table-sm">
				...
				@foreach ($orders as $key => $cat)
					<tr>
						...
						<td>{{ $cat->office_name ? $cat->office_name : "VIA WEB" }}</td>
						...
						<td>
							<div class="option_row">
								<!-- <a href="#" class="btn btn-secondary text_option py-0 px-1" data-target="#answer-modal" data-toggle="modal"><i class="fa-solid fa-file-pen"></i> Responder</a> -->
								@if($cat->status == 1)
								<a href="" data-index="{{ $cat->id }}" data-new_status="3" data-target="#modal-delete-" data-toggle="modal" class="btn btn-info action py-0 px-1" title="RECIBIR"><i class="fa-regular fa-envelope-open notPointerEvent"></i></a>
								@endif
		  ...
		  if (result.value) {
            ...
            $.ajax({
                ...
                success: function(e){
                    ...
                    setTimeout(function(){
                    	location.replace('/admin/recibidos');
                     }, 1000);
...
document.querySelector('#solicitudes_report')
	.addEventListener('click', () => {
		window.open(`/admin/solicitudes-report?incio=${startDate}&fin=${endDate}&name=${document.querySelector('input[name="searchText"]').value}&status=1`);
	});
